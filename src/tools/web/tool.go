...
func NewWebTool(workingDir string) llms.Tool {
	...
	return &core.Tool{
		Name:        "web_browser",
		Description: "Navigate the web, pull content from pages, interact with buttons using a headless browser, and search the web via the Brave Search API.",
...
- Parameters:
  * action (required): The action to perform: "navigate", "click", "get_content", "save_content", or "web_search"
...
- Selector errors: Use browser developer tools to verify CSS selector syntax.`,
		Parameters: []core.Parameter{
			{
				Name:        "action",
				Type:        "string",
				Description: "The action to perform: 'navigate', 'click', 'get_content', 'save_content', or 'web_search'",
				Required:    true,
		...
		Handler: func(agentContext map[string]any, args map[string]any) llms.ToolReturn {
			...
			default:
				return core.NewErrorResponse(fmt.Sprintf("unknown action: %s. Valid actions are: navigate, click, get_content, save_content, web_search", action))
			}
		},
	}
}
