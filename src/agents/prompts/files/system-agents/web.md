...
- Step 1: Identify what web operations are needed (navigation, clicking, or content pulling)
- Step 2: Determine the appropriate action from the 4 available: navigate, click, get_content, save_content
- Step 3: For content pulling: use the default workflow - Navigate → save_content → Report file path
- Step 4: For clicking: use click action with CSS selector
- Step 5: Use get_content only when explicitly requested by main agent for immediate content access
- Step 6: Execute the web operation using the web_browser tool
- Step 7: Return results clearly, always including file paths when using save_content
