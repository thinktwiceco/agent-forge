...
When `CanSpawnSubagent` is true, the agent gets the built-in **spawn_subagent** tool. Enable it in YAML with `agent.spawn_subagent: true` ([`builder.Config`](../../src/builder/agentBuilder.go)), or in code with `agents.NewBuilder(...).WithSpawnSubagent().Build()`. It creates a **short-lived** child agent with the same LLM engine, a **subset** of the parent’s tools (by name), plus built-in **meta** / **expand**, and attaches the **todo** plugin when it is registered. The parent passes `prompt` and `tools: string[]`; the tool returns immediately with a **spawn_id** and the subagent result arrives as a **follow-up turn** on the same conversation via the turn queue.
